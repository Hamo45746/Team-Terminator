...
def create_vertex_list(cube):
    ...
    vertex_list[0][0] = vertex.fiducials[cube-1].x0 #x0 position
    vertex_list[0][1] = vertex.fiducials[cube-1].y0 #y0 position
    vertex_list[1][0] = vertex.fiducials[cube-1].x1 #x1 position
    vertex_list[1][1] = vertex.fiducials[cube-1].y1 #y1 position
    vertex_list[2][0] = vertex.fiducials[cube-1].x2 #x2 position
    vertex_list[2][1] = vertex.fiducials[cube-1].y2 #y2 position
    vertex_list[3][0] = vertex.fiducials[cube-1].x3 #x3 position
    vertex_list[3][1] = vertex.fiducials[cube-1].y3 #y3 position
    return vertex_list

def get_centre(vertex_list):
    '''
    ...
    '''
    cube_x0 = vertex_list[0][0]
    cube_y0 = vertex_list[0][1]
    cube_x2 = vertex_list[2][0]
    cube_y2 = vertex_list[2][1]
    return [(cube_x0 + cube_x2) / 2, (cube_y0 + cube_y2) / 2]

def set_transform(p, wanted_id):
    for i, tag in enumerate(p.transforms):
        if(wanted_id == tag.fiducial_id):
            T_camtag = tag
            ...
            robot_to_tag = np.matmul(np.linalg.inv(camera_to_robot),\
                camera_to_tag_1)

            desired_y = robot_to_tag[1][3]
            desired_x = robot_to_tag[0][3]

            theta1 = np.arctan(desired_y/desired_x)
            best_transform = robot_to_tag

            return best_transform

def get_cube_distance(cube1_centre, cube2_centre):
    '''
    Returns the distance between 2 cubes using their centre vertices
    '''

    return abs(np.sqrt((cube2_centre[1]-cube1_centre[1])**2+\
                (cube2_centre[0]-cube1_centre[0])**2))
    
def get_closest_vertex(vertex_list):
    '''
    Returns the closest vertex to the robot
    Inputs:
    - vertex_list (list): a list of the cube vertices
    '''
    vertex_closest = [0,0]
...
def T_C2R(p):
    ...
    Performs other operations based on state of the robot. In state 2 will\
         publish desired x,y
    coordinates of end effector in robot frame to reach the optimal aruco tag. 
    In state 7 publishes the current robot pose. In state 4 publishes desired \
        pose for
    ...
    if (state==0):
        ...
        if(len(p.transforms) == 1):
            cube1_id = vertex.fiducials[0].fiducial_id
            best_transform = set_transform(p,cube1_id)

        elif(len(p.transforms) == 2):
            cube1_id = vertex.fiducials[0].fiducial_id
            cube1_vertex_list = create_vertex_list(1)
            cube2_id = vertex.fiducials[1].fiducial_id
            cube2_vertex_list = create_vertex_list(2)

            cube1_centre = get_centre(cube1_vertex_list)
            cube2_centre = get_centre(cube2_vertex_list)

            cube1to2 = get_cube_distance(cube1_centre,cube2_centre)

            if(cube1to2 > distance):
                best_transform = set_transform(p,cube1_id)
        
        elif(len(p.transforms) == 3):
            cube1_id = vertex.fiducials[0].fiducial_id
            cube1_vertex_list = create_vertex_list(1)
            cube2_id = vertex.fiducials[1].fiducial_id
            cube2_vertex_list = create_vertex_list(2)
            cube3_id = vertex.fiducials[2].fiducial_id
            cube3_vertex_list = create_vertex_list(3)

            cube1_centre = get_centre(cube1_vertex_list)
            cube2_centre = get_centre(cube2_vertex_list)
            cube3_centre = get_centre(cube3_vertex_list)

            cube1to2 = get_cube_distance(cube1_centre,cube2_centre)
            cube1to3 = get_cube_distance(cube1_centre,cube3_centre)
            cube2to3 = get_cube_distance(cube2_centre,cube3_centre)

            if((cube1to2 > distance) and (cube1to3 > distance)):
                best_transform = set_transform(p,cube1_id)
            elif((cube1to2 > distance) and (cube2to3 > distance)):
                best_transform = set_transform(p,cube2_id)
            elif((cube1to3 > distance) and (cube2to3 > distance)):
                best_transform = set_transform(p,cube3_id)

        elif(len(p.transforms) == 4):
            cube1_id = vertex.fiducials[0].fiducial_id
            cube1_vertex_list = create_vertex_list(1)
            cube2_id = vertex.fiducials[1].fiducial_id
            cube2_vertex_list = create_vertex_list(2)
            cube3_id = vertex.fiducials[2].fiducial_id
            cube3_vertex_list = create_vertex_list(3)
            cube4_id = vertex.fiducials[3].fiducial_id
            cube4_vertex_list = create_vertex_list(4)

            cube1_centre = get_centre(cube1_vertex_list)
            cube2_centre = get_centre(cube2_vertex_list)
            cube3_centre = get_centre(cube3_vertex_list)
            cube4_centre = get_centre(cube4_vertex_list)

            cube1to2 = get_cube_distance(cube1_centre,cube2_centre)
            cube1to3 = get_cube_distance(cube1_centre,cube3_centre)
            cube1to4 = get_cube_distance(cube1_centre,cube4_centre)
            cube2to3 = get_cube_distance(cube2_centre,cube3_centre)
            cube2to4 = get_cube_distance(cube2_centre,cube4_centre)
            cube3to4 = get_cube_distance(cube3_centre,cube4_centre)

            if((cube1to2 > distance) and (cube1to3 > distance) and (cube1to4 > distance)):
                best_transform = set_transform(p,cube1_id)           
            elif((cube1to2 > distance) and (cube2to3 > distance) and (cube2to4 > distance)):
                best_transform = set_transform(p,cube2_id)
            elif((cube1to3 > distance) and (cube2to3 > distance) and (cube3to4 > distance)):
                best_transform = set_transform(p,cube3_id)
            elif((cube1to4 > distance) and (cube2to4 > distance) and (cube3to4 > distance)):
                best_transform = set_transform(p,cube4_id)   
    
        if is_moving == 1:
            pub_valid_cube.publish(0)
        else:
            current_pose = Pose()
            current_pose.position.x = best_transform[0][3]
            current_pose.position.y =  best_transform[1][3]
            current_pose.position.z = best_transform[2][3]
...
if __name__=='__main__':
    ...
    rospy.init_node('tag_selector_3A')
    rospy.spin()  